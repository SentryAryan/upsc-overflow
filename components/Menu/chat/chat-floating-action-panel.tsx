...
>(({ children, className, title, mode }, ref) => {
  ...
  return (
    <motion.button
      ...
      className={cn(
        "flex h-9 items-center rounded-md border px-3 text-sm font-medium shadow-lg border-border bg-backgound! hover:scale-105 transition-all ease-in hover:text-primary duration-300",
        className
      )}
      onClick={handleClick}
...
>(({ children, className }, ref) => {
  ...
  const getPosition = () => {
    if (!triggerRect) return { left: "50%", bottom: "50%" };
    ...
    const bottom = triggerRect.top + scrollY + 8; // 8px gap
